import React from "react";
import "./HeaderTab.css";
import {
  FiChevronRight,
  FiChevronDown,
  FiGrid,
  FiFilter,
  FiMapPin,
  FiBell,
} from "react-icons/fi";
import { useState } from "react";
function HeaderTab() {
  const [prof, setopenprof] = useState("false");

  return (
    <div>
      <header className="top-header">
        <div className="left">
          <h1>Gogaga</h1>
          <p>☰ </p>
        </div>

        <div className="right">
          <div className="header-right">
            <div>
              <FiGrid />
            </div>

            <div className="icon-box green">
              <FiFilter />
            </div>
            <span className="badge green">520</span>

            <div className="icon-box blue">
              <FiMapPin />
            </div>
            <span className="badge blue">1</span>

            <div className="icon-box red">
              <FiBell />
            </div>
            <span className="badge red">15</span>
          </div>
          <div className="Profile-bg">
            <img
              src="data:image/jpeg;base64,"
              className="Profile-bg-img"
              alt="name"
            />
            <span> Girish Kumar </span>
            <span>{prof ? <FiChevronDown /> : <FiChevronRight />}</span>
          </div>
        </div>
      </header>
    </div>
  );
}

export default HeaderTab;
